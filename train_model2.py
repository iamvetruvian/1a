...
MODEL_PKL = 'heading_classifier_lgbm.pkl'
LABEL_ENCODER_PKL = 'label_encoder.pkl'
EMBEDDING_MODEL = 'sentence-transformers/all-MiniLM-L6-v2'

print("Loading data...")
df = pd.read_csv(TRAIN_CSV)
if df['is_bold'].dtype == bool:
    df['is_bold'] = df['is_bold'].astype(int)
elif df['is_bold'].dtype == object:
    df['is_bold'] = df['is_bold'].apply(lambda x: 1 if x in [True, 'True', 'true', '1'] else 0)
...
feature_cols = ['font_size', 'is_bold', 'x0', 'y0'] + emb_cols
X = df[feature_cols]
y = df['label_enc']
...
# ---- LGBMClassifier (recommended: guaranteed compatible) ----
